...
export const useAIConfigStore = defineStore('aiConfig', () => {
  ...
  const systemPrompt = ref('你是一個專業的 YouTube 影片助手，可以幫助使用者理解和討論影片內容。請用繁體中文回答。')
  ...
  // 配置資訊
  const config = computed((): AIConfig => ({
    apiKey: apiKey.value,
  ...
  const resetConfig = () => {
    ...
    systemPrompt.value = '你是一個專業的 YouTube 影片助手，可以幫助使用者理解和討論影片內容。請用繁體中文回答。'
